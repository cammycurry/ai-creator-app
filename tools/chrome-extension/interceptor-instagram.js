...
// Each captured item includes the post shortcode so the content script can match
// the correct video to the reel the user is actually viewing.
(function () {
  "use strict";
  ...
  // Walk the parsed JSON to find media objects (XDTMediaDict) that have
  // both a "code" (shortcode) and "video_versions" or "image_versions2".
  ...
  function walkForMedia(obj, items, seenCodes) {
    ...
    // A media object has "code" + ("video_versions" or "image_versions2")
    if (typeof obj.code === "string" && obj.code.length > 3) {
      ...
      // Extract audio/music metadata — Instagram nests this in several possible locations
      let audio = null;
      ...
      if (musicAsset) {
        audio = {
          ...
          audioId: musicAsset.audio_cluster_id || musicAsset.audio_id || musicAsset.ig_artist?.pk || null,
        };
      } else if (originalSound) {
        audio = {
          title: originalSound.original_audio_title || originalSound.audio_asset_info?.title || null,
      ...
      if (Array.isArray(obj.video_versions) && obj.video_versions.length > 0 && !seenCodes.has(code + "_video")) {
        ...
        let best = versions.find((v) => v.type === 104);
        if (!best) best = versions.find((v) => v.type === 101);
        if (!best) best = versions[0];
        if (best?.url) {
          const url = cleanUrl(best.url);
          if (isCdnUrl(url)) {
            items.push({ url, type: "video", code, pk, username, caption, audio, width: best.width || 0, height: best.height || 0 });
          }
        }
      }
      ...
      if (obj.image_versions2?.candidates && !seenCodes.has(code + "_image")) {
        ...
        if (candidates[0]?.url) {
          ...
          if (isCdnUrl(url)) {
            items.push({ url, type: "image", code, pk, username, caption, audio, width: candidates[0].width || 0, height: candidates[0].height || 0 });
          }
        }
      }
      ...
      if (typeof obj.video_url === "string" && obj.video_url && !seenCodes.has(code + "_legacy")) {
        ...
        if (isCdnUrl(url)) {
          items.push({ url, type: "video", code, pk, username, caption, audio, width: 0, height: 0 });
        }
      }
    }
    ...
    if (Array.isArray(obj)) {
      ...
      for (const key of Object.keys(obj)) {
        if (typeof obj[key] === "object" && obj[key] !== null) {
          walkForMedia(obj[key], items, seenCodes);
        }
      }
    }
  }
  ...
  function regexFallback(text, items, seenCodes) {
    // Try to find "code":"XXX" near "video_versions" — less reliable but better than nothing
      ...
      try {
        ...
        if (best?.url) {
          ...
          if (isCdnUrl(url)) {
            ...
            items.push({ url, type: "video", code, pk: null, username: null, width: best.width || 0, height: best.height || 0 });
          }
        }
      } catch {}
    }
  ...
  function bridgeToContentScript(items) {
    ...
    for (const item of items) {
      // Dedup by code+type (one video per shortcode)
      if (item.code && queue.some((q) => q.code === item.code && q.type === item.type)) continue;
    ...
    console.log(`[mc-ig] Bridged ${items.length} media (${queue.length} total in queue)`);
  }
  ...
  setTimeout(() => {
    ...
    if (ogVideo?.content) {
      ...
      bridgeToContentScript([{ url: ogVideo.content, type: "video", code, pk: null, username: null, width: 0, height: 0 }]);
    }
  }, 1500);
})();
