#!/usr/bin/env npx tsx
/**
 * Wizard Prompt Test Round 4: H and I
 * H = TRUE full realism (all applicable anchors including under-eye, uneven skin, camera artifacts)
 *     + explicit white bg + flat lighting + no jewelry/nails/logos
 * I = H + gorgeous language
 * Testing whether the "ugly" anchors we dropped for content actually HELP for wizard
 */
...
// The FULL realism block for wizard — all applicable anchors
// Dropped: jewelry, nail detail, environmental noise (white bg), lighting imperfection (studio)
// Added back: under-eye texture, uneven skin tone, camera artifacts
const FULL_REALISM = [
  "Visible pores on forehead, nose, and cheeks.",
  "Subtle natural skin texture with fine detail.",
  "Baby hairs at temples and flyaways catching the light.",
  "Slight under-eye shadows and fine lines.",
  "Natural uneven skin tone with subtle variation.",
  "Defined facial features.",
  "Fabric texture visible on clothing with natural stretch, no logos, no branding, plain solid fabric.",
  "Slight camera edge softness and subtle shadow noise.",
  "Pure white seamless studio background. Flat even studio lighting, no directional shadows, no color cast.",
  "Photorealistic.",
].join(" ");
...
const strategies: { id: string; label: string; buildPrompt: StrategyFn }[] = [
  {
    id: "H-true-full-realism",
    label: "TRUE Full Realism (all anchors incl under-eye + uneven skin + camera artifacts)",
    buildPrompt: (basePrompt) => {
      const withoutRealism = basePrompt.replace(`${CAMERA}. ${REALISM_BASE}.`, "").trim();
      return wrapWithSilhouette(`${withoutRealism} ${CAMERA}. ${FULL_REALISM}`);
    },
  },
  {
    id: "I-true-full-gorgeous",
    label: "TRUE Full Realism + Gorgeous (all anchors + hot language)",
    buildPrompt: (basePrompt, traits) => {
      ...
      const withoutRealism = basePrompt.replace(`${CAMERA}. ${REALISM_BASE}.`, "").trim();
      return wrapWithSilhouette(`${withoutRealism} ${hotLine} ${CAMERA}. ${FULL_REALISM}`);
    },
  },
];
...
async function main() {
  ...
  console.log("═══ WIZARD PROMPT TEST ROUND 4: H + I (TRUE FULL REALISM) ═══");
  console.log(`${total} images`);
  console.log("Anchors: pores, skin texture, baby hairs, under-eye, uneven skin, defined features, fabric (no logos), camera artifacts, white bg, flat lighting");
  ...
  if (!html.includes("H-true-full-realism")) {
    html = html.replace(
      `{id:"G-full-realism-gorgeous",label:"G: Full Real + Hot",desc:"All 6 anchors + white bg + flat lighting + gorgeous language"},`,
      `{id:"G-full-realism-gorgeous",label:"G: Full Real + Hot",desc:"All 6 anchors + white bg + flat lighting + gorgeous language"},
  {id:"H-true-full-realism",label:"H: TRUE Full",desc:"ALL anchors (incl under-eye, uneven skin, camera artifacts) + white bg"},
  ...
  console.log("Now 10 columns: A B C D E D-Fixed F G H I");
}
